...
public class TestUtil {
	...
	public static void conductBeanComparison(Object expected, Object observed) throws ArrayComparisonFailure,
		...
		assertEquals(String.format("Beans must be the same class."), expected.getClass().getName(), observed.getClass()
				.getName());
		...
		for (Method method : getterMethods) {
			...
			if (method.getReturnType().getName().startsWith("[I")) {
				...
				assertEquals(String.format("Value for field %s must be the same.", fieldName), method.invoke(expected), method
						.invoke(observed));
			}
		}
	}
